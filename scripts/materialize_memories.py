...
VAULT_MEMORY = Path.home() / "Dev/Obsidian-Second-Brain/Claude-Memory"
TODAY = date.today().isoformat()

# ── Project card map (same as materialize_sessions.py) ────────────────────────
...
PROJECT_CARD_MAP = {
    ...
    "periodical-parser": "Projects/server (category)/periodical-parser/_PROJECT_CARD",
    ...
    "write-pipeline": "Projects/server (category)/whatbox/morning-reader/_PROJECT_CARD",
...
def slugify(project: str) -> str:
    """Normalize project slug: lowercase, replace _ with -, strip leading -."""
...
def build_project_note(project_slug: str, fragments: list[tuple[str, str]]) -> str:
    ...
    """
    card_path = PROJECT_CARD_MAP.get(
        project_slug, f"Projects/{project_slug}/_PROJECT_CARD"
    )
    count = len(fragments)
